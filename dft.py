...
img = cv.imread("Lenna.png")  # image to perform processing on
...
for i in range(len(with_err_diffuse)):
    for j in range(len(with_err_diffuse[0])):
        ...
        if(j+1)<len(with_err_diffuse[0]):
            with_err_diffuse[i][j+1] = with_err_diffuse[i][j+1] + (error*(7/16))     ##error diffused to right pixel
        if(i+1)<len(with_err_diffuse) and (j-1)>=0:
            with_err_diffuse[i+1][j-1] = with_err_diffuse[i+1][j-1] + (error*(3/16))     ##error diffused to bottom left pixel
        if(i+1)<len(with_err_diffuse):
            with_err_diffuse[i+1][j] = with_err_diffuse[i+1][j] + (error*(5/16))         ##error diffused to bottom pixel
        if(i+1)<len(with_err_diffuse) and (j+1)<len(with_err_diffuse[0]):
            with_err_diffuse[i+1][j+1] = with_err_diffuse[i+1][j+1] + (error*(1/16))     ##error diffsued to bottom right pixel
